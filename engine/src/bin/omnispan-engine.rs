...
use omnispan::config::{env_or_default, env_u16, env_u64};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let worker_rpc_timeout_ms = env_u64("WORKER_RPC_TIMEOUT_MS", 30000);
    ...
    let worker_client = DirectWorkerClient::new(worker_endpoint.clone(), worker_rpc_timeout_ms);
